...
let currentSelectedSize = null; // NEW: Tracks the selected size
...
function toggleTheme() { document.body.classList.toggle('dark-mode'); localStorage.setItem('viratDarkModeMain', document.body.classList.contains('dark-mode')); document.getElementById('theme-toggle').className = document.body.classList.contains('dark-mode') ? 'fas fa-sun' : 'fas fa-moon'; }

document.addEventListener("DOMContentLoaded", () => {
    if (localStorage.getItem('viratDarkModeMain') === 'true') { document.body.classList.add('dark-mode'); document.getElementById('theme-toggle').className = 'fas fa-sun'; }
...
async function fetchProducts() {
    try {
        ...
        products = data ? Object.keys(data).map(key => ({ id: key, ...data[key] })).reverse() : [];
        renderProducts(products, 'home-grid', 16);
        renderProducts(products, 'shop-grid', 1000);
    } catch (e) {}
}
...
function renderProducts(productList, containerId, limit = 100) {
    ...
    productList.slice(0, limit).forEach(p => {
        let soldOut = p.status === 'Out of Stock' || parseInt(p.stock) <= 0;
...
// NEW: Function to handle size clicks
...
function openProduct(id) {
    ...
    // NEW: Reset size selection when opening a new product
...
function shareProduct() {
    ...
    const text = `Hey! Check out this beautiful ${currentProduct.name} for just ₹${currentProduct.finalPrice} at Virat Fashion! 🛍️\n\n${window.location.href}`;
...
async function processLogin() {
    ...
    try {
        ...
        if(existingUser) { currentUser = existingUser; currentUser.dbKey = existingKey; } 
            ...
            currentUser = { name: name, phone: phone, userId: `VF${Math.floor(10000 + Math.random() * 90000)}`, banned: false, cart: [] };
        ...
        if(currentUser.cart && currentUser.cart.length > 0 && cart.length === 0) { cart = currentUser.cart; localStorage.setItem('viratCart', JSON.stringify(cart)); document.getElementById('cart-counter').innerText = cart.length; }
        localStorage.setItem('viratUser', JSON.stringify(currentUser));
        updateProfileUI(); closeLogin(); navigate('profile-view'); showToast("Logged in!");
    } catch (error) {}
}

function logoutUser() { currentUser = null; localStorage.removeItem('viratUser'); window.location.reload(); }
...
function syncCart() { if(currentUser && currentUser.dbKey) { fetch(`${DB_URL}users/${currentUser.dbKey}.json`, { method: 'PATCH', body: JSON.stringify({ cart: cart }) }); currentUser.cart = cart; localStorage.setItem('viratUser', JSON.stringify(currentUser)); } }

function addToCartFromDetail() {
    if(!currentProduct) return;
    if(currentProduct.status === 'Out of Stock' || parseInt(currentProduct.stock) <= 0) return showToast("Sorry, product is out of stock!", "error");
    
    // NEW: Block adding to cart if size is not selected
    if(!currentSelectedSize) return showToast("Please select a size (M, L, XL, XXL)!", "error");

    cart.push({ ...currentProduct, name: `${currentProduct.name} (Size: ${currentSelectedSize})`, price: currentProduct.finalPrice || currentProduct.price });
    document.getElementById('cart-counter').innerText = cart.length;
    localStorage.setItem('viratCart', JSON.stringify(cart)); syncCart(); showToast("Added to cart!");
}
...
function removeFromCart(index) { cart.splice(index, 1); localStorage.setItem('viratCart', JSON.stringify(cart)); renderCart(); document.getElementById('cart-counter').innerText = cart.length; syncCart(); }
...
function openCheckoutModal(mode) {
    ...
    // NEW: Block Buy Now if size is not selected
    if (checkoutMode === 'single' && !currentSelectedSize) {
        return showToast("Please select a size (M, L, XL, XXL)!", "error");
    }

    // Include the size in the single product checkout name
    let singleCheckoutItem = currentProduct ? { ...currentProduct, name: `${currentProduct.name} (Size: ${currentSelectedSize})` } : null;
    let orderItems = checkoutMode === 'single' ? [singleCheckoutItem] : cart;
    ...
    let upiLinkStr = `upi://pay?pa=${dynamicUpiId}&pn=Virat%20Fashion&am=${checkoutFinalTotal}&cu=INR`;
...
document.getElementById('checkout-form').addEventListener('submit', async function(e) {
    ...
    // NEW: Update the items string for the Admin dashboard to show the size
    let itemsString = checkoutMode === 'single' ? `${currentProduct.name} (Size: ${currentSelectedSize})` : cart.map(i => i.name).join(", ");
    ...
    try {
        ...
        closeCheckout();
        if(checkoutMode === 'cart') { cart = []; localStorage.setItem('viratCart', JSON.stringify(cart)); syncCart(); document.getElementById('cart-counter').innerText = 0; }
